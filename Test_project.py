...
class QuizApp:
    ...
    def create_grade_screen(self):
        ...
        self.clear_screen()
        header = tk.Label(self.root, text="Select Your Grade",
                          font=("Arial", 24, "bold"), bg="#e6f0fa", fg="#003366")
        header.pack(pady=40)

        frame = tk.Frame(self.root, bg="#e6f0fa")
        frame.pack()
        ...
        tk.Label(self.root, text="Choose your grade to begin.",
    ...
    def create_subject_screen(self, grade):
        self.grade = grade
        self.clear_screen()
        tk.Label(self.root, text=f"Select a Subject (Grade {grade})",
        ...
        ttk.Button(self.root, text="Back", command=self.create_grade_screen, cursor="hand2").pack(pady=20)
        self.add_footer()
    ...
    def create_unit_screen(self, subject):
        ...
        self.clear_screen()
        tk.Label(self.root, text=f"{subject} - Select a Unit",
                 font=("Arial", 22, "bold"), bg="#e6f0fa", fg="#003366").pack(pady=40)
    ...
    def load_question_limit_screen(self, unit_id):
        ...
        entry_frame = tk.Frame(self.root, bg="#e6f0fa")
        ...
        # --- BUTTONS ---
        btn_frame = tk.Frame(self.root, bg="#e6f0fa")
        ...
        tk.Label(self.root, text="Choose the amount of questions.",
    ...
    def add_footer(self):
        footer = tk.Label(
            self.root,
            text="Developed By Emperor   |   Supervised By Amir Watoo",
    ...
    def start_quiz(self, unit):
        self.unit = unit  # store unit name
        ...
        # Now assign it to the instance variable
        self.unit_id = unit_id

        # Ask how many questions user wants
        question_limit = self.load_question_limit_screen(self.unit_id)
        if not question_limit:
            return

        # Load only LIMIT number of random questions
        self.questions = self.load_limited_questions(unit_id, question_limit)
        ...
        if not self.questions:
            ...
            return
        # Reset quiz state
        self.shuffled_sets = {}          # ← Reset shuffled options
        self.current_q = 0
        self.score = 0
        self.answers = {}
        self.review_mode = False
    ...
    def load_question_screen(self):
        ...
        # --- HEADER ---
        tk.Label(self.root, text=f"Subject: {self.subject}",
        ...
        progress = ttk.Progressbar(self.root, value=progress_value,
        ...
        # --- QUESTION HEADER ---
        header_frame = tk.Frame(self.root, bg="#e6f0fa")
        ...
        # --- QUESTION CARD ---
        card_shadow = tk.Frame(self.root, bg="#cdd7e0")
        ...
        # --- OPTIONS SECTION ---
        options_frame = tk.Frame(self.root, bg="#e6f0fa")
        ...
        for opt in shuffled_options:
            ...
            opt_frame.bind("<Button-1>", lambda e, v=opt, rb_widget=rb: click_option(e, v, rb_widget))
            rb.bind("<Button-1>", lambda e, v=opt, rb_widget=rb: click_option(e, v, rb_widget))
        ...
        # Add new trace and store the ID
        self.option_trace = self.selected_option.trace_add("write", lambda *args: on_select_callback())
        ...
        # --- NAVIGATION ---
        nav_frame = tk.Frame(self.root, bg="#e6f0fa")
        ...
        if self.review_mode:
            nav_panel = tk.Frame(self.root, bg="#e6f0fa")
    ...
    def create_unit_screen_back(self):
        ...
        if confirm:
            self.create_unit_screen(self.subject)
    ...
    def show_result(self):
        ...
        tk.Label(self.root, text="Quiz Completed!",
                 font=("Arial", 22, "bold"), bg="#e6f0fa", fg="#003366").pack(pady=40)
        tk.Label(self.root, text=f"Grade: {self.grade}", font=("Arial", 14), bg="#e6f0fa").pack()
        tk.Label(self.root, text=f"Subject: {self.subject}", font=("Arial", 14), bg="#e6f0fa").pack()
        tk.Label(self.root, text=f"{self.unit}", font=("Arial", 14, "italic"), bg="#e6f0fa").pack(pady=5)
        ...
        ttk.Button(self.root, text="Review Answers", command=self.start_review, cursor="hand2").pack(pady=10)
        ttk.Button(self.root, text="Back to Units", command=lambda: self.create_unit_screen(self.subject), cursor="hand2").pack(pady=10)
        ttk.Button(self.root, text="Exit", command=self.confirm_exit, cursor="hand2").pack(pady=10)
        ...
        tk.Label(self.root, text=f"Percentage: {percent:.1f}%", 
